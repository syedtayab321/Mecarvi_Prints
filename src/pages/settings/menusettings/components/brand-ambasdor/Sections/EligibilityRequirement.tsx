...
const EligibilityRequirementsData = [
  {
    id: 1,
...
export default function EligibilityRequirements() {
  ...
  const filteredPartners = EligibilityRequirementsData.filter((work) =>
  ...
  return (
    <main className="min-h-max sm:min-w-[97%] mt-8 sm:m-8 p-8 bg-white w-auto rounded-2xl shadow-custom max-w-[100%]">
      <div className="flex justify-between items-center border-b pb-4 mb-4">
        <h2 className="text-4xl font-bold ">Eligibility Requirements</h2>
        <AddRequirements />
      </div>
        ...
        <table className="min-w-full bg-white text-xl text-gray-700">
          ...
          <tbody>
            {filteredPartners.length > 0 ? (
              filteredPartners.map((requirements) => (
                <tr
                  key={requirements.id}
                  className="border-t hover:bg-gray-50 cursor-pointer"
                >
                  <td className="px-6 py-4">{requirements.title}</td>
                  <td className="px-6 py-4">{requirements.description}</td>
                  <td className="px-6 py-4 flex gap-4 text-right">
                    <EditRequirements data={requirements} />
                    <DeleteRequirements />
                  </td>
                </tr>
              ))
