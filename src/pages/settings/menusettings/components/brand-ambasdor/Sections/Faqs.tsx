...
import { AddFaqs, DeleteFaqs, EditFaqs } from "../PopUps";

const faqsData = [
  {
    id: 1,
    question: "What is a company?",
    answer: "https://www.mecarvicorporation.com",
  },
];
...
export default function Partners() {
  ...
  const filteredPartners = faqsData.filter((faqs) =>
  ...
  return (
    <main className="min-h-max sm:min-w-[97%] mt-8 sm:m-8 p-8 bg-white w-auto rounded-2xl shadow-custom max-w-[100%]">
      <div className="flex justify-between items-center border-b pb-4 mb-4">
        <h2 className="text-4xl font-bold uppercase">FAQS</h2>
        <AddFaqs />
      </div>
        ...
        <table className="min-w-full bg-white text-xl text-gray-700">
          ...
          <tbody>
            {filteredPartners.length > 0 ? (
              filteredPartners.map((faqs) => (
                <tr
                  key={faqs.id}
                  className="border-t hover:bg-gray-50 cursor-pointer"
                >
                  <td className="px-6 py-4">{faqs.question}</td>
                  <td className="px-6 py-4">{faqs.answer}</td>
                  <td className="px-6 py-4 flex gap-4 text-right">
                    <EditFaqs faqsData={faqs} />
                    <DeleteFaqs />
                  </td>
                </tr>
              ))
